...
| gd                                     | 跳转到光标所在字符串第一次定义的位置,非常适合看代码,跳到变量定义位置(局部变量) |
| gD                                     | 跳转到光标所在字符串第一次定义的位置,非常适合看代码,跳到变量定义位置(全局变量) |
...
| 按键          | 效果                                                         |
| ------------- | ------------------------------------------------------------ |
| :e [filename] | 打开另一个文件,注意,使用e打开另一个文件前需要将当前文件的修改写入,或者执行:e!,打开另一个文件后,执行:q退出时,会将多个打开的文件都退出 |
| :bn           | 这种方式打开多个文件时切换到下一个文件                       |
| :bp           | 这种方式打开多个文件时切换到上一个文件                       |
...
| [ctrl]+w+= | 恢复成两个窗口相等的尺寸 |
| [ctrl]+w++和[ctrl]+w+- | 横屏增加和减小尺寸 |
| [ctrl]+w+>和[ctrl]+w+< | 竖屏向右和向左移动窗口 |
