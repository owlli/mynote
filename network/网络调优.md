...
# 查看网卡多队列的hash算法
[root@localhost ~]#ethtool -n enp2s0 rx-flow-hash tcp4
TCP over IPV4 flows use these fields for computing Hash flow key:
IP SA
IP DA
L4 bytes 0 & 1 [TCP/UDP src port]
L4 bytes 2 & 3 [TCP/UDP dst port]
# 看上面的输出，一个tcp会话，肯定只会发到同一个队列里
# 20260407，做测试发现，同一个tcp会话，两个方向的包会发到两个队列中。测试方法：A机器开启网卡多队列（英特尔I211网卡，龙蜥8.10系统，当前网卡不支持收包队列和发包队列独立，所以设置的是combined的值为2），A机器和B机器使用网线直连，在B机器使用tcpreplay回放一个tcp会话的pcap包，观察A机器的两个网卡队列的中断响应数（cat /proc/interrupts | grep enp3s0），发现两个队列数的增长分别对应pcap文件中请求和响应数。
...
系统默认启用，这个服务会监控各个cpu的中断负载情况，动态地调整中断号和cpu号的绑定策略，让中断均匀地负载到各个cpu上。

但是貌似网卡队列数超过cpu核数一半时，不会将每个网卡队列软中断都单独占用一个核，比如在某个现场，32核设备（16核超线程），对一块万兆网卡开启24个网卡队列，最多只有16核在处理软中断，有8个软中断复用了其他软中断的cpu。

但是这种服务有个缺点，动态地调整有可能把一个中断号分配到多个cpu增加了跨核访问开销，所以流量极高的服务器环境，应该关闭irqbalance服务，使用自己的手动中断绑定策略。
